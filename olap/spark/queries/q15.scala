...
val revenue = lineitems.filter($"l_shipdate" >= "1998-10-10").filter($"l_shipdate" < "1999-01-10").select($"l_suppkey", decrease($"l_extendedprice", $"l_discount").as("with_discount")).groupBy($"l_suppkey").agg(sum($"with_discount").as("total_value"))
